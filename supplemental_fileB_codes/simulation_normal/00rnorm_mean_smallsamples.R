...
for (i in seq_len(150)){
    print(i)
    negda <- data.frame(matrix(rnorm(500*20,mean=10000,sd=100), nrow=20, byrow=TRUE))
    colnames(negda) <- paste0("N", seq_len(ncol(negda)))
    poida1 <- data.frame(matrix(rnorm(500*10,mean=10000-i, sd=100), 
                                nrow=10, byrow=TRUE))
    poida2 <- data.frame(matrix(rnorm(500*10,mean=10000+i, sd=100),
                                nrow=10, byrow=TRUE))
...
#write.table(kwfprfnr,paste0("./result/kwfprfnr_", seednum,"_norm_mean_ss.txt"), sep="\t", 
#			quote=FALSE, row.names=FALSE, col.names=TRUE)
#
#ancombcfprfnr <- do.call("rbind", ancombcfprfnr) %>% rownames_to_column(var="t")
#write.table(ancombcfprfnr,paste0("./result/ancombcfprfnr_", seednum,"_norm_mean_ss.txt"), sep="\t",
#           quote=FALSE, row.names=FALSE, col.names=TRUE)
#
#dafprfnr <- do.call("rbind", dafprfnr) %>% rownames_to_column(var="t")
#write.table(dafprfnr,paste0("./result/dafprfnr_", seednum,"_norm_mean_ss.txt"), sep="\t", 
#			quote=FALSE, row.names=FALSE, col.names=TRUE)
#
#ancfprfnr <- do.call("rbind", ancfprfnr) %>% rownames_to_column(var="t")
#write.table(ancfprfnr, paste0("./result/ancfprfnr_", seednum, "_norm_mean_ss.txt"), sep="\t",
#	                            quote=FALSE, row.names=FALSE, col.names=TRUE)
#
#metafprfnr <- do.call("rbind", metafprfnr) %>% rownames_to_column(var="t")
#write.table(metafprfnr, paste0("./result/metafprfnr_", seednum, "_norm_mean_ss.txt"), sep="\t",
#	                            quote=FALSE, row.names=FALSE, col.names=TRUE)
#
#lefsefprfnr <- do.call("rbind", lefsefprfnr) %>% rownames_to_column(var="t")
#write.table(lefsefprfnr, paste0("./result/lefsefprfnr_", seednum, "_norm_mean_ss.txt"), sep="\t",
#	                            quote=FALSE, row.names=FALSE, col.names=TRUE)

lindafprfnr <- do.call("rbind", lindafprfnr) %>% rownames_to_column(var="t")
write.table(lindafprfnr, paste0("./result/lindafprfnr_", seednum, "_norm_mean_ss.txt"), sep="\t",
           quote=FALSE, row.names=FALSE, col.names=TRUE)

zicoseqfprfnr <- do.call("rbind", zicoseqfprfnr) %>% rownames_to_column(var="t")
write.table(zicoseqfprfnr, paste0("./result/zicoseqfprfnr_", seednum, "_norm_mean_ss.txt"), sep="\t",
                        quote=FALSE, row.names=FALSE, col.names=TRUE)

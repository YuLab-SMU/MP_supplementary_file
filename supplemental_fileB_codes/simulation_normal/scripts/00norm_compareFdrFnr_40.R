...
##################### different mean ##################################
#######################################################################
data <- read.table("../result/kwfprfnr_1024_norm_mean.40.txt", header=T, sep="\t")
#data2 <- read.table("../result/kwfprfnr_512_norm_mean.txt", header=T, sep="\t")
#data <- rbind(data, data2)
data$methods <- "KW"
data <- data %>% select(c("t","FPR","FNR","methods"))
dat <- read.table("../result/dafprfnr_1024_norm_mean.40.txt", header=T, sep="\t")
...
dt <- read.table("../result/ancombcfprfnr_1024_norm_mean.40.txt", header=T, sep="\t")
...
meseq <- read.table("../result/metafprfnr_1024_norm_mean.40.txt", header=T, sep="\t")
...
anmean <- read.table("../result/ancfprfnr_1024_norm_mean.40.txt", header=T, sep="\t")
anmean$methods <- "ANCOM"
anmean <- anmean %>% select(c("t","FPR","FNR","methods"))

lefse <- read.table("../result/lefsefprfnr_1024_norm_mean.40.txt", header=T, sep="\t")
lefse$methods <- "LEfSe"
lefse <- lefse %>% select(c("t", "FPR", "FNR", "methods"))

lindamean <- read.table('../result/lindafprfnr_1024_norm_mean.40.txt', header=T, sep='\t')
lindamean$methods <- 'LinDA'
lindamean <- lindamean %>% select(c('t', 'FPR', 'FNR', 'methods'))

ziconseqmean <- read.table('../result/zicoseqfprfnr_1024_norm_mean.40.txt', header=T, sep='\t')
...
##################### different sd  ##################################
######################################################################
dasd <- read.table("../result/kwfprfnr_1024_norm_sd.40.txt", header=T, sep="\t")
#dasd2 <- read.table("../result/kwfprfnr_1024_norm_sd.txt", header=T, sep="\t")
#dasd <- rbind(dasd, dasd2)
dasd$methods <- "KW"
dasd <- dasd %>% select(c("t", "FPR", "FNR", "methods"))

datsd <- read.table("../result/dafprfnr_1024_norm_sd.40.txt", header=T, sep="\t")
...
dtsd <- read.table("../result/ancombcfprfnr_1024_norm_sd.40.txt", header=T, sep="\t")
...
meseqsd <- read.table("../result/metafprfnr_1024_norm_sd.40.txt", header=T, sep="\t")
...
ansd <- read.table("../result/ancfprfnr_1024_norm_sd.40.txt", header=T, sep="\t")
ansd$methods <- "ANCOM"
ansd <- ansd %>% select(c("t", "FPR", "FNR", "methods"))

lefsesd <- read.table("../result/lefsefprfnr_1024_norm_sd.40.txt", header=T, sep="\t")
lefsesd$methods <- "LEfSe"
lefsesd <- lefsesd %>% select(c("t", "FPR", "FNR", "methods"))

lindasd <- read.table('../result/lindafprfnr_1024_norm_sd.40.txt', header=T, sep='\t')
lindasd$methods <- 'LinDA'
lindasd <- lindasd %>% select(c('t', 'FPR', 'FNR', 'methods'))

ziconseqsd <- read.table('../result/zicoseqfprfnr_1024_norm_sd.40.txt', header=T, sep='\t')
...
damsd <- read.table("../result/kwfprfnr_1024_norm_meansd.40.txt", header=T, sep="\t")
...
datmsd <- read.table("../result/dafprfnr_1024_norm_meansd.40.txt", header=T, sep="\t")
...
dtmsd <- read.table("../result/ancombcfprfnr_1024_norm_meansd.40.txt", header=T, sep="\t")
...
meseqmsd <- read.table("../result/metafprfnr_1024_norm_meansd.40.txt", header=T, sep="\t")
...
anmsd <- read.table("../result/ancfprfnr_1024_norm_meansd.40.txt", header=T, sep="\t")
anmsd$methods <- "ANCOM"
anmsd <- anmsd %>% select(c("t", "FPR", "FNR", "methods"))

lefsemsd <- read.table("../result/lefsefprfnr_1024_norm_meansd.40.txt", header=T, sep="\t")
lefsemsd$methods <- "LEfSe"
lefsemsd <- lefsemsd %>% select(c("t", "FPR", "FNR", "methods"))

lindamsd <- read.table('../result/lindafprfnr_1024_norm_meansd.40.txt', header=T, sep='\t')
lindamsd$methods <- 'LinDA'
lindamsd <- lindamsd %>% select(c('t', 'FPR', 'FNR', 'methods'))

ziconseqmsd <- read.table('../result/zicoseqfprfnr_1024_norm_meansd.40.txt', header=T, sep='\t')
...
svg("../figures/norm_compare_point.40.svg", width=7.5, height=3)
p
dev.off()
...
svg("../figures/norm_compare_boxplot.40.svg", width=8, height=4)
p_box
dev.off()

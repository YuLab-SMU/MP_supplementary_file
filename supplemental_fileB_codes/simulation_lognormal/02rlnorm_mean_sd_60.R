...
for (i in seq(1, 3,by=0.01)){
    print(i)
    subneg1 <- data.frame(matrix(rlnorm(500*15,meanlog=10-2,sdlog=i), 
                                        nrow=15, byrow=TRUE))
    subneg2_3 <- data.frame(matrix(rlnorm(500*30, meanlog=10, sdlog=i),
                                        nrow=30,byrow=TRUE))
    subneg4 <- data.frame(matrix(rlnorm(500*15, meanlog=10+2, sdlog=i),
                                        nrow=15,byrow=TRUE))
    negda <- rbind(subneg1,subneg2_3,subneg4)
    colnames(negda) <- paste0("N", seq_len(ncol(negda)))
    poida1 <- data.frame(matrix(rlnorm(500*30,meanlog=10-2, sdlog=i), 
                                        nrow=30, byrow=TRUE))
    poida2 <- data.frame(matrix(rlnorm(500*30,meanlog=10+2, sdlog=i),
                                        nrow=30, byrow=TRUE))
...
#write.table(kwfprfnr,paste0("./result/kwfprfnr_", seednum,"_rlnorm_mean_sd.60.txt"), sep="\t",
#			quote=FALSE, row.names=FALSE, col.names=TRUE)
#
#ancfprfnr <- do.call("rbind", ancfprfnr) %>% rownames_to_column(var="t")
#write.table(ancfprfnr, paste0("./result/ancfprfnr_", seednum, "_rlnorm_mean_sd.60.txt"), sep="\t",
#			quote=FALSE, row.names=FALSE, col.names=TRUE)
#
#ancombcfprfnr <- do.call("rbind", ancombcfprfnr) %>% rownames_to_column(var="t")
#write.table(ancombcfprfnr, paste0("./result/ancombcfprfnr_", seednum, "_rlnorm_mean_sd.60.txt"), sep="\t",
#            quote=FALSE, row.names=FALSE, col.names=TRUE)
#
#dafprfnr <- do.call("rbind", dafprfnr) %>% rownames_to_column(var="t")
#write.table(dafprfnr,paste0("./result/dafprfnr_", seednum,"_rlnorm_mean_sd.60.txt"), sep="\t",
#			quote=FALSE, row.names=FALSE, col.names=TRUE)
#
#metafprfnr <- do.call("rbind", metafprfnr) %>% rownames_to_column(var="t")
#write.table(metafprfnr, paste0("./result/metafprfnr_", seednum, "_rlnorm_mean_sd.60.txt"), sep="\t",
#			quote=FALSE, row.names=FALSE, col.names=TRUE)
#
#lefsefprfnr <- do.call("rbind", lefsefprfnr) %>% rownames_to_column(var="t")
#write.table(lefsefprfnr, paste0("./result/lefsefprfnr_", seednum, "_rlnorm_mean_sd.60.txt"), sep="\t",
#	                            quote=FALSE, row.names=FALSE, col.names=TRUE)

lindafprfnr <- do.call("rbind", lindafprfnr) %>% rownames_to_column(var="t")
write.table(lindafprfnr, paste0("./result/lindafprfnr_", seednum, "_rlnorm_mean_sd.60.txt"), sep="\t",
           quote=FALSE, row.names=FALSE, col.names=TRUE)

zicoseqfprfnr <- do.call("rbind", zicoseqfprfnr) %>% rownames_to_column(var="t")
write.table(zicoseqfprfnr, paste0("./result/zicoseqfprfnr_", seednum, "_rlnorm_mean_sd.60.txt"), sep="\t",
                               quote=FALSE, row.names=FALSE, col.names=TRUE)

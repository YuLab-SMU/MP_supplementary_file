...
We found the some differential clades contains the species that were not detected the closely related species that were not detected on the previous differential analysis, such as OTU_454/OTU_97 (both belong to Clostridiaceae SMB53), OTU_152/OTU_233 (both belong to Lachnospira), which suggested the balance transform can improve the detection of differential signals by accumulating the small consilient differences at a broad resolution.
...
f.box <- mpse.balance.node %>%
    ...
    facet_wrap(pseudolabel~., ncol = 1, strip.position = 't', scales = 'free') +
    scale_fill_manual(values = c('orange', 'deepskyblue')) +

...
This package provides `mp_diff_analysis` to detect the biomarker. And the result (with `action = "get"`) can be visualized by *ggdiffbox*, *ggdiffclade*, *ggeffectsize*, *ggdifftaxbar* and *mp_plot_diff_res* (with `action = "add"`), or displayed manually using `ggtree` [@ggtree2017] and `ggtreeExtra` [@ggtreeExtra].
...
### visualization of differential results (with action = "get") by `ggdiffbox` {#chapter22.6.2}
...
```{r fig.width=6, fig.height=8, fig.align="center", warning=FALSE, message=FALSE, fig.cap="(ref:DiffBoxplot)", diffbox}
...
### visualization of different results by `ggeffectsize` {#chapter2.6.4}
...
```{r fig.width=5, fig.height=8, fig.align="center", warning=FALSE, message=FALSE, fig.cap="(ref:LDAplot)", plotES}
...
## Interoperable with the existing computing ecosystem {#chapter2.7}
...
### Alpha diversity analysis in 16s level {#chapter3.1.1}
...
### Beta diversity analysis in 16s level {#chapter3.1.2}
...
### Composition of the taxonomy in 16s level {#chapter3.1.3}
...
```{r}
KO.da <- read.table("./data/CD_RF_microbiome/biscuit_mgs_KOs.tsv", header=TRUE, sep = "\t", row.names=1, check.names=F)
...
```{r, fig.width = 10, fig.height = 10, fig.cap = '(ref:DiffKEGG)', G}
mpseKO %<>% 
   mp_diff_analysis(
     .abundance = Abundance,
     force = TRUE,
...
```{r, fig.width = 8, fig.height = 10, fig.cap="(ref:MGSBeta)", MGSBeta}
mpseMGS %<>%
   mp_decostand(
      .abundance = Abundance,
      method = "hellinger"
...
```{r, message = FALSE, fig.width = 10, fig.height = 10, fig.cap = "(ref:DiffMGS)", DiffMGS}
mpseMGS %<>%
    mp_diff_analysis(
       .abundance = Abundance,
       force = TRUE,
...
```{r, message = FALSE, fig.cap = '(ref:MosquitoaAlpha)', MosquitoaAlpha}
p.alpha <- mpse %>%
...
```{r, message = FALSE, warning = FALSE, fig.width=12, fig.height=4.6, fig.cap = '(ref:MosquitoCCA)', MosquitoCCA}
